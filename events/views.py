...
ADMIN_SECRET_CODE = "ADMIN2026"  # Change this to your desired admin code
...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        elif roll_no and StudentProfile.objects.filter(roll_no=roll_no).exists():
            messages.error(request, 'A student with this Roll Number already exists.')
        else:
            user = User.objects.create_user(username=username, email=email, password=password, first_name=first_name, last_name=last_name)
            if roll_no or semester or year or branch or course:
